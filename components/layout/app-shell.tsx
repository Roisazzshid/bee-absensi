...
const NAV_ITEMS: { label: string; href: string }[] = [
  ...
  { label: "Izin & cuti", href: "/izin-cuti" },
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:flex md:pb-0">
      ...
      <aside className="hidden min-h-screen w-72 flex-col border-r border-outline-variant/70 bg-surface p-6 md:flex">
        <Brand />
        <div className="mt-8 flex items-center gap-3 rounded-2xl bg-surface-container-low p-3">
        ...
        <button
          ...
          className="mt-auto rounded-xl px-4 py-3 text-left text-sm font-bold text-error hover:bg-red-50"
        >
          Keluar
        </button>
      </aside>
      ...
      <div className="min-w-0 flex-1">
        ...
        <main className="mx-auto w-full max-w-3xl px-5 py-6 md:px-8 md:py-10">{children}</main>
      </div>

      {/* ── Mobile bottom nav ── */}
      <nav
        className="fixed inset-x-0 bottom-0 z-20 flex h-20 items-center justify-around rounded-t-2xl border-t border-surface-container bg-surface-container-lowest px-3 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] md:hidden"
...
function Brand({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={compact ? "font-bold text-primary" : "text-xl font-bold text-primary"}>
        Bee Absensi
      </span>
    </div>
  );
...
function Avatar({ initials }: { initials: string }) {
  return (
    <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-surface-container-high text-xs font-bold text-primary">
      {initials}
    </span>
  );
}
  ...
  return (
    <button
      ...
      className={join(
        ...
        mobile
          ? "flex min-w-14 flex-col items-center rounded-xl px-2 py-2 text-[10px]"
