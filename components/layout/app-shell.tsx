...
const NAV_ITEMS: { label: string; href: string; icon: (active: boolean) => ReactNode }[] = [
  {
    ...
    href: "/",
    icon: (active) => (
      <svg className={`size-6 ${active ? "text-primary" : "text-muted-foreground"}`} viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth={active ? 0 : 1.8}>
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:flex md:pb-0">
      ...
      <aside className="hidden min-h-screen w-72 flex-col border-r border-border bg-card p-6 shadow-sm md:flex">
        <Brand />
        <div className="mt-8 flex items-center gap-3 rounded-2xl bg-muted/50 p-3">
        ...
        <button
          ...
          className="mt-auto flex items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-bold text-red-600 dark:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/50"
        >
      ...
      <div className="min-w-0 flex-1 flex flex-col min-h-screen">
        ...
        {/* Desktop top bar just for Theme Toggle (Optional, can be positioned absolute top right, or a header) */}
        <div className="hidden md:flex justify-end p-4 lg:p-6 absolute top-0 right-0 z-10">
           <ThemeToggle />
        </div>
...
function Brand({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <div className="flex size-8 items-center justify-center rounded-xl overflow-hidden bg-background ring-1 ring-border">
        <img
